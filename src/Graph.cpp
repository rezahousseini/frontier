...
void 
Graph::freezeVertForever(int vName)
{
  int i, len;
  Vertex *verP;
...
void 
Graph::defrostOneVert(int vName)
{
  int i, len;
  Vertex *verP;
...
Edge& 
Graph::returnEdgeByEnds(int v1, int v2)
{
  int i, v01, v02;
  for(i=0;i<NumEdge;i++)
    {
...
Edge& 
Graph::returnEdgeByName(int eName)
{
  int i;
  for(i=0;i<NumEdge;i++)
...
Vertex& 
Graph::returnVertByName(int vName)
{
  int i;
  for(i=0;i<NumVert;i++)
...
void 
Graph::augment(Vertex *v3p, Edge *edgep)
{
  int wt;
  Vertex *vp;
  Edge *ep;
...
//distribut Edge
int 
Graph::distribute0(Edge &edge, std::ostream &file2)
{
  ...
  for(i=1;i<=NumVert;i++)
    {
      Vertex new_ver;
      new_ver.setName(i);
      Vweight=2+ (int) (3*distribution(generator));
  ...
  for(i=1;i<=NumVert;i++)
    {
      for(j=i+1;j<=NumVert;j++)
        {
          if(distribution(generator)+propability>=1.0)
            {
              Edge new_edg;
              NumEdge++;
              new_edg.setName(NumEdge);
              Eweight=1+ (int) (2*distribution(generator));
...
std::ostream& 
operator <<(std::ostream& os, const Graph& gra)
{
  int i;
  Vertex currVert;
  Edge currEdge;
  ...
  if(gra.NumVert==0 && gra.NumEdge==0)
    {
  ...
  for(i=1;i<=gra.NumEdge;i++)
    {
      currEdge=gra.edges.retrieve(i);
  ...
  for(i=1;i<=gra.NumVert;i++)
    {
      currVert=gra.vertices.retrieve(i);
      os<<currVert<<std::endl; 
      totalVertW+=currVert.returnWeight();
      if(vertFlow[currVert.returnName()]!=currVert.returnEstFlow())
      os<<"Flows do not match for vertex "<<currVert.returnName()
        <<": Curr Flow="<<vertFlow[currVert.returnName()]<<" Est Flow="
        <<currVert.returnEstFlow()<<std::endl;
    }
