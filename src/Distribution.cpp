...
//! \file mfa6Dist.cpp
//! \brief Contains the distribute-edge flow method and the pushoutside method
...
bool 
specialPushCase (Graph &graph0, Cluster &C)
{
...
int 
pushOutside (Cluster &C, Graph &F, Graph &graph0, std::ostream &file1, 
             std::ostream &file2, int nextVerName)
{
   int i, a, K, nFront;  
   int coreName, v1Name, v2Name, targetName;
...
int pushOutside0(Graph &F, std::ostream &file2, int nextVerName)
{
   ...
   for(i=1;i<=numTrivial;i++)
   {
      ...
      for(j=1;j<=nVerF;j++)
      {
	     ...
	     if(checkSpecialExtensionCase(CCF, std::cout, nextVerName))
		outEdge.setWeight(2);
...
Cluster* 
distributeVert (Vertex &v1, Graph &F, Graph &graph0, std::ostream &file1, 
         ...
         if(edgeName&&F.hasVert(v2Name))
         {
            ...
            else
            {
                ...
                if(k>0)
                {
                   ...
                   {
                      newTree=getCluster(F, graph0, nextEdgeName, nextVerName);
 		      ...
 		      file2 << newTree << std::endl;
                   ...
                   {
                      if(extension(F, graph0, file1, file2, singleVertex, nextVerName)==1)
                      {
			   ...
			   return newTree;
                      ...
                      a=pushOutside0(F, file2, nextVerName);//only trivial dense subgraph
                      if(a>0)
                      {
                          newTree=getCluster(F, graph0, nextVerName, nextEdgeName);
...
//! distribute Cluster
Cluster& 
distributeCl (Cluster &C, Graph &F, Graph &graph0, std::ostream &file1, 
              std::ostream &file2, int singleVertex, int nextVerName, 
              int nextEdgeName)
{
   Cluster *newTree;
   ...
   if(v1Name>0&&v1Name<=singleVertex) // distributing single vertex
   {
      ...
      v1=C.returnCore();
      newTree=distributeVert(v1, F, graph0, file1, file2, singleVertex, 
   ...
   else  // distributing a cluster
   {
       ...
       k=pushOutside(C, F, graph0, file1, file2, nextVerName);
       if(k>0)
       {
          newTree=getCluster(F, graph0, nextEdgeName, nextVerName);
          file2<<"Found new Cluster:"<<std::endl;
