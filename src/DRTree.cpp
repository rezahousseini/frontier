...
//! \file DRTree.cpp
//! \brief Methods for distribution including distribute-forest, 
...
Cluster&
distributeTree (Cluster &aDRtree, Graph &F, Graph &graph0, std::ostream &file1, 
                std::ostream &file2, int indentation, int singleVertex, 
...
void explicitChild(Cluster &newTree, List<int> &nameList,List<Cluster> &forest)
{
   ...
   std::ofstream outf;
//   outf.open("explicitChild.out", std::ios::app);
   outf << "The beginning ++++++++++ " << std::endl << "The cluster is: " << std::endl;
   outf << newTree << std::endl;
...
int getClusterType(Graph &graph0, Cluster &theCluster)
{
	...
	std::ofstream outf;
	outf.open("getclusterType.out", std::ios::app);
...
getClusterType (Graph &graph0, Cluster &theCluster)
{
  ...
  for(int i=1;i<=vLen; i++)
	...
	if(numDis ==1 && graph0.returnDimen()==2) return 5;
	if(numDis ==1 && graph0.returnDimen()==3) return 2;
	return 111;
}
...
{	
	int v1, v2;
	std::ofstream outf;
//	outf.open("specialCluster.out", std::ios::app);
...
//! a cluster is a sub cluster of another if all of its original vertices are 
//! original vertices in the larger cluster. Subset operator
bool 
canCombine (Cluster &c, Cluster &cl0, Graph &F, Graph &graph0, int singleVertex,
            int nextVerName, int nextEdgeName)
{
	int i, nOrig0, nOrig;
	List<int> origV0;
	...
	std::ofstream outf;
//	outf.open("canCombine.out", std::ios::app);
	outf << "The beginning:" << std::endl;
	...
	DR_Trees=distributeForest(DR_Trees, F1, graph1, std::cout, std::cout, 0,0,
                            singleVertex, nextVerName, nextEdgeName);
 	getTrivial(DR_Trees, F1, singleVertex, nextVerName);
                      ...
                      int singleVertex, int nextVerName, int nextEdgeName)
{
   ...
   std::ofstream outf;
//   outf.open("implicit.out", std::ios::app);
   outf<< "(The Beginning)The newTree's name is:" << newTree.returnName() << std::endl;
   numOfTree=forest.returnLen();
   for(i=1;i<=numOfTree;i++)
   {
      aChild=popMaxDepth(forest);
      if(canCombine(aChild, newTree, F, graph0, singleVertex, nextVerName, nextEdgeName))
      {
	  ...
	  outf << F << std::endl;
          ...
          temp=getCluster(F, graph0, nextVerName, nextEdgeName);
               ...
               int singleVertex, int nextVerName, int nextEdgeName)
{
   ...
   nameList=newTree.returnOrig(); //vertex name list forms a new cluster
	std::ofstream outf;
//	outf.open("addchild.out", std::ios::app);
   ...
   implicitChild(newTree, nameList, DR_Trees, F, graph0, singleVertex, 
                 nextVerName, nextEdgeName);
   printForest(DR_Trees, std::cout, 0);

   // look implicit child in popedTrees
   implicitChild(newTree, nameList, popedTrees, F, graph0, singleVertex, 
                 nextVerName, nextEdgeName);
...
int checkClusterForBananas(Graph &graph0, Cluster &theCluster)
{
	...
	std::ofstream outf;
//	outf.open("checkBananas.out", std::ios::app);
	outf << " The beginning:" << std::endl;
...
void setAlias(Graph &graph0, Cluster &theCluster)
{
   ...
   int alias=-1;
	...
	std::ofstream outf;
//	outf.open("setAlias.out", std::ios::app);
...
void pseudoMinDense(Cluster &theCluster, Graph &graph0)
{
   ...
   std::ofstream outf;
//	outf.open("pseudoMinDense.out", std::ios::app);
	...
	isChange=false;
   	...
   	for(i=1;i<=length;i++)
		for(j=i+1;j<=length;j++)
		{
			...
			outf << tempCluster.children.retrieve(i) << std::endl;
			outf<<"Cluster 2: "<<theCluster.children.retrieve(j).returnName()<<std::endl;
			outf << tempCluster.children.retrieve(j) << std::endl;
...
bool 
canMerge (Graph &graph0, Cluster &child1, Cluster &child2, int singleVertex, 
          int nextVerName, int nextEdgeName)
{
	std::ofstream outf;
//	outf.open("canMerge.out", std::ios::app);
	//create vertex list
	List<int> verList;
	...
	DR_Trees=distributeForest(DR_Trees, F1, graph1, dd1, dd2, 0,0,singleVertex, 
                            nextVerName, nextEdgeName);
 	getTrivial(DR_Trees, F1, singleVertex, nextVerName);
	outf<<"Contents of new DRTrees"<<std::endl;
	printForest(DR_Trees, outf, 0);	
...
int 
modifyTree (Graph &graph0, Cluster &newC, int singleVertex, int nextVerName, 
            int nextEdgeName)
{
...
//	outf.open("modifyTree.out", std::ios::app);
	...
	for(int i=1; i<=newC.children.returnLen();i++)
	{
		...
		for(int j=i+1; j<=newC.children.returnLen();j++)
		{
			...
			outf << child2 << std::endl;
			if(canMerge(graph0, child1, child2, singleVertex, nextVerName, nextEdgeName))
			{
	...
	for(int i=1; i<=newC.children.returnLen(); i++)
		outf << newC.children.retrieve(i) << std::endl;
...
int setConstrain(Graph &G, Cluster &theCluster)
{
	...
	List<int> overList;
	getOverlapList(G, theCluster, overList, 0, 0);
...
//! Distribute all the clusters in DR_Trees assuring that the groups set by the 
//! user and found in the output DR_Dag. Both distributeForest and 
//! distributeCluster work recursively together to perform this process
List<Cluster>&
distributeForest (List<Cluster> &DR_Trees, Graph &F, Graph &graph0, 
                  std::ostream &file1, std::ostream &file2, int groupID, 
                  ...
                  int nextEdgeName)
{
   ...
   while(DR_Trees.returnLen()>0)
   {
      ...
      if(extension(F, graph0, firstTime, groupID, file1, file2, singleVertex, nextVerName))
      {
         	file2<<"Found new Cluster by Extension:"<<std::endl;
         	newTree=*getCluster(F, graph0, nextVerName, nextEdgeName);
         	file2 << newTree << std::endl;
         	file2<<"----------------end----------------"<<std::endl;
      }
      else
      {	 
	 std::cout<<"In distributeCL path"<<std::endl;
         aTree=popMinDepth(DR_Trees, nextVerName);  // item is deleted in DR_Trees
	 ...
	 if(graph0.returnDimen()==3)
	 {
         ...
         newTree=distributeCl(aTree, F, graph0, file1, file2, singleVertex, nextVerName, nextEdgeName);
      }
    ...
    file2 << newTree << std::endl;
          ...
          printForest(DR_Trees, file2, 0);
          addChild(newTree, popedTrees, DR_Trees, F, graph0, singleVertex, 
                   nextVerName, nextEdgeName);
    ...
    std::cout << graph0 << std::endl;
	  ...
	  if(newTree.returnDepth()>1)
	          modifyTree(graph0, newTree, singleVertex, nextVerName, nextEdgeName);
...
void 
getTrivial (List<Cluster> &DRTrees, Graph &graph0, int singleVertex, 
            int nextVerName)
{
