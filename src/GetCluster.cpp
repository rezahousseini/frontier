...
//! \file GetCluster.cpp
//! \brief contains two methods: copying  graph and sequential extensions for 
//!        the DR-planner
...
bool 
canExtend (Graph &F, Graph &graph0, List<Cluster> &DR_Trees, int groupID, 
           List<int> &groupV)
{
...
//! performs a sequential extension on the labeled section of F, sequential 
//! extensions are described in the attached paper documentation
int 
extension (Graph &F, Graph &graph0, std::ostream &file1, std::ostream &file2, 
           int singleVertex, int nextVerName)
{
  Vertex v0, v1;
  Edge edge;
  List<Edge> newEdges, oldEdges;  // put new edges F when extend a vertex
  int j, nVertF, nLabelF, possible, extEdgeNum;
  int v0Name, v1Name;
  int eWeight, newEdgeLen, oldEdgeLen;
  int notIncidCount=0;
  ...
  for(v0Name=1;v0Name<nextVerName;v0Name++) // v0--new vertex to be extended
    {
      ...
      if(F.hasVert(v0Name))
        {
          v0=F.returnVertByName(v0Name);
          // v0 in F, but NOT in new cluster
          if(v0.returnLabel()==0&&(!v0.isFrozen())) possible=1; 
      ...
      if(possible==1)  // v0 is possible to be extended
        {
          ...
          for(j=1;j<=nVertF;j++)           // for every vertex v1 in F
                  ...
                  if(edge.returnWeight()!=0)
                    {
                      ...
                      file2<<"Update:  extEdgeNum="<<extEdgeNum<<" eWeight="
                           <<eWeight<<std::endl;
                    }
                }
            }
	        ...
	        test=checkSpecialExtensionCase(F, file2, nextVerName);
...
int 
extension (Graph &F, Graph &graph0, bool firstTime, int groupID, 
           ...
           int singleVertex, int nextVerName)
{
  if(firstTime && groupID>0)
	...
	return extension (F, graph0, file1, file2, singleVertex, nextVerName);
}


//! After distribute has been run, Graph F contains vertices labeled for the 
//! creation of a new cluster this method isolates those vertices, performs 
//! any reductions to the interior vertices and edges as necessary, and 
//! creates a new Cluster object, returning a pointer to it for more on the 
//! specifics of this reduction process, see attached paper documentation
Cluster * getCluster(Graph &F, Graph &graph0, int nextVerName, int nextEdgeName)
{
    ...
    int i, j, nVerF, nFront=0, nIncid, nVerInner=0;
